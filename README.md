...
- 依据原始 Logo 轮廓制作的 Three.js 蓝白立体云标，支持鼠标拖动、手机横向滑动旋转，手机纵向滑动仍可滚页。
- Logo 持续缓慢自动转动，拖动时也不中断，不提供暂停按钮；拖动和方向键调整旋转中心，Home 键恢复默认方向。
- 最终角度限制在左右 ±50°、上下 ±22°，为自动摆动预留余量，始终展示正面。
- 遵循系统减少动态效果设置，系统要求减少动态效果时仍可手动旋转；不可见时停止渲染。
